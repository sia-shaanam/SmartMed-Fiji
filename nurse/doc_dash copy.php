...
<body>
  ...
  <nav class="nav-bar">
    ...
    <h3>Your Doctor ID: <?php echo $doctor_id; ?></h3>
  <div class="dropdown-wrapper" style="display: flex; align-items: center; gap: 50px;">
    ...
    <h1><?php echo htmlspecialchars($doctor['specialization']); ?></h1>
  <h4>Department: <?php echo htmlspecialchars($doctor['department']); ?></h4>
  <h4>Contact Information: <?php echo htmlspecialchars($doctor['contact_information'] ?? 'N/A'); ?></h4> 
    <h4>Email: <?php echo htmlspecialchars($doctor['email']); ?></h4>
         ...
         <div class="card"><i class="fas fa-calendar-check"></i><h3><?php  echo "Today's Confirmed Appointments: " . $todays_confirmed_appointments;?>    
                </h3><p>A doctor’s greatest strength lies not only in their knowledge but in their unwavering commitment.</p></div>
                 
                <div class="card"><i class="fas fa-calendar-check"></i><h3>  <?php  
                    echo "Total Confirmed Appointments : " . $total_confirmed_appointments;
                    ?>           
               </h3><p>In the hands of a dedicated doctor, medicine becomes a symphony of care and empathy.</p></div>
       
      

        <div class="card"><i class="fas fa-procedures"></i><h3><?php     
             echo "Total Patients: " . htmlspecialchars($total_patient);     ?></h3><p>Behind every cure, there’s a doctor who never gave up on finding it.</p></div>
...
if ($details_stmt) {
    ...
    if ($todays_appointments > 0) {
        ...
        echo "<table id='appointmentTable' border='1'>
                <tr>
                    <th>Patient Name</th>
