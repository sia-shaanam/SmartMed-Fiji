...
if ($result->num_rows === 1) {
    ...
    // Step 2: Use the doctor_id to count total appointments
    $appointment_sql = "SELECT COUNT(*) AS total_appointments FROM appointments WHERE doctor_id = ?";
    ...
    if ($appointment_stmt) {
        ...
        $appointment_stmt->bind_result($total_appointments);
...
<body>
  ...
  <div class="patient-card" style="background-color:#04c9f5;">
    ...
    <div class="appointments">
        ...
        // Assume you have $doctor_id set from a session or previous query
        // Get today's date
        $today = date('Y-m-d');

        // Fetch confirmed appointments for the given doctor and today
        $sql = "SELECT COUNT(*) AS total_confirmed FROM appointments WHERE status = 'Confirmed' AND appointment_date = ? AND doctor_id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$today, $doctor_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        $total_confirmed = $result['total_confirmed'];

        echo "<h3>Appointments for Doctor ID: " . htmlspecialchars($doctor_id) . "</h3>";
        echo "<br>";
        echo "<h4>Today's Confirmed Appointments: " . htmlspecialchars($total_confirmed) . "</h4>";
        ?>
...
<div>
 ...
 <h1 style="padding:10px; font-size:30px;">Master Calendar</h1>
    ...
    <div class="section-header">
        ...
        <div class="filters">
            <form method="GET" action="">
                ...
                <a href="appointment_form.html">
                <button type="button" class="add-appointment" style="color:white; background-color: #04c9f5;">+ Add Remarks</button></a>
