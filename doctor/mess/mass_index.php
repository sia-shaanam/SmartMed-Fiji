...
// Step 1: Retrieve the doctor_id and name for the logged-in user
$sql = "SELECT doctor_id, name FROM doctors WHERE user_id = ?";
$stmt = $conn->prepare($sql);

if (!$stmt) {
    die('Error: Failed to prepare doctor query: ' . htmlspecialchars($conn->error));
}

$stmt->bind_param("i", $user_id); // Assuming user_id is an integer
$stmt->execute();
$stmt->bind_result($doctor_id, $doctor_name); // Bind both doctor_id and doctor_name (name)
$stmt->fetch();

// Check if a doctor_id was found
if (!$doctor_id) {
    die("Error: No doctor found for the logged-in user.");
}

$stmt->close(); // Close the statement for fetching doctor info
...
// Retrieve the latest message per patient for the logged-in user
$user_id = $_SESSION['user_id'];
$sql = "
    SELECT m.patient_id, p.firstName, m.message, m.sent_at 
    FROM messages m
    JOIN patient p ON m.patient_id = p.patient_id
    WHERE m.user_id = ?
    AND m.sent_at = (SELECT MAX(sent_at) FROM messages WHERE patient_id = m.patient_id AND user_id = m.user_id)
            ...
            <?php if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <div class="message-card">
                        <div class="info">
                            <h4><?php echo htmlspecialchars($row['firstName'], ENT_QUOTES, 'UTF-8'); ?></h4>
                            <p><?php echo htmlspecialchars($row['message'], ENT_QUOTES, 'UTF-8'); ?></p>
                            <span><?php echo htmlspecialchars($row['sent_at'], ENT_QUOTES, 'UTF-8'); ?></span>
                        </div>
                        <div class="actions">
                            <button class="chat" onclick="window.location.href='chat.php?patient_id=<?php echo $row['patient_id']; ?>'">Chat</button>
                        </div>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
